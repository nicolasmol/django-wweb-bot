...
class Ad {
  ...
  private static async validate(adText: string): Promise<boolean | string> {
    if (Str.forbiddenWordsAsRegex.some((word) => RegExp(word).exec(adText))) {
    ...
    if (chatGPTanswer === null) {
      const nameAndAge = RegExp(Str.startsWithNameAndAgeRegex).exec(adText);

      if (nameAndAge === null) {
        return Str.sorryDoesntSeemsToStartwithNameAndAgeText;
      }

      const forCheckAdText = adText.replace(Str.startsWithNameAndAgeRegex, "");
  ...
  private static async validateWithChatGPT(
    ...
    const messages = [
      {
        role: "user",
        content: Str.chatGTPpofileValidationPrePromptText.replace("%1", adText),
      },
    ];
